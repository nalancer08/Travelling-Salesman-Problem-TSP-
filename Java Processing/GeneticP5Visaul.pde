...
void draw() {
  ...
  System.out.println("Entre al draw");
	background(220);
	...
	if (best != null) {
		...
		line(points[best[0]].x, points[best[0]].y, points[best[best.length - 1]].x, points[best[best.length - 1]].y);
	}

	fill(0);
  stroke(0, 250, 0); // Color de la linea
  strokeWeight(5); // Ancho de la linea
	for (int i = 0; i < lists.size(); i++) {
		ellipse(lists.get(i).x, lists.get(i).y, i == 0 ? 16 :8, i == 0 ? 16 :8); // Dibujamos los puntitos (los ultimos dos parametros, son su w & h)
    noStroke();
	}
